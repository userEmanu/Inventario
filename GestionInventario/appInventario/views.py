...
def registrarUsuario(request):
    try:
        ...
        with transaction.atomic():
            ...
            thread.start()
            return redirect("/inicioAdmin/", retorno)
...
def login(request):
    ...
    if result['success']:
        usernamee= request.POST["txtUser"] 
        passworde = request.POST["txtPassword"]
...
def asistenteSolicitudes(request):
    if request.user.is_authenticated:
        datosSesion={"user": request.user,"rol":request.user.groups.get().name, "solicitud": DetalleSolicitud.objects.all()}
